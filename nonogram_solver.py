...
class ClueExtra:
    ...
    def check_dominate(self, boxes: Block, fixed=False) -> bool:
        if boxes.length > self._value:
            return None

        if fixed and boxes.length != self._value:
            return None
        ...
        section = BlockSection(boxes.begin, boxes.end)
        section &= self._boxes
        if section.length > 0:
            return True
        ...
        if block.length > self._value:
            return None

        # This is not possible.
        # if block not in self._candidates:
        #     return None
...
class LineSolver:
    def __init__(self, clues: typing.Tuple[int], content: typing.List[CellType], line: Line=None):
        self._clues = clues
        self._content = content
        self._line = line or 'line'
    ...
    def _mark_cell(self, index: int, value: CellType):
        if index < 0 or index >= self._width:
            raise ParadoxError(f'{self._line} cell {index + 1} index out of range')
    ...
    def _trim_finished(self, begin: int, end: int, clue_begin: int, clue_end: int, step: int):
        while begin != end:
            if self._content[begin] is None:
                ...
                raise ParadoxError(f'{self._line} cell {begin + 1} cannot be {CellType.BOX}')
            else:
                begin += step
    ...
    def _handle_known_boxes(self, known_boxes) -> bool:
        ...
        while index < len(known_boxes):
            ...
            for clue in candidate_clues:
                dominate = clue.check_dominate(block, fixed)
                if dominate:
                    clues.clear()
                    clues.append(clue)
                    clue.confirm_boxes(block)
            ...
            if (fixed_at_begin or fixed_at_end) and not fixed:
                ...
                begin = block.begin if fixed_at_begin else block.end - clue_min
                end = block.end if fixed_at_end else block.begin + clue_min

                clue_max = max(clue.value for clue in clues)
                if clue_max == clue_min:
                    if fixed_at_begin and end < self._remain_cell.end:
                        self._set_space(end)
                        updated = True
                    elif fixed_at_end and begin > self._remain_cell.begin:
                        self._set_space(begin - 1)
                        updated = True

                more = clue_min - block.length
                if more > 0:
                    if fixed_at_begin:
                        for i in range(more):
                            self._mark_cell(block.end + i, CellType.BOX)
                        ...
                        for i in range(more):
                            self._mark_cell(block.begin - i - 1, CellType.BOX)
                        if index > 0 and known_boxes[index - 1].block.end >= begin:
                            begin = known_boxes[index - 1].block.end
                            del known_boxes[index - 1]
                            index -= 1

                    known_boxes[index] = BoxBlockAndClues(Block(begin, end), clues)
                    updated = True
                    continue
        ...
        for index in range(len(known_boxes) - 1):
            ...
            if not next_clues:
                raise ParadoxError(f'{self._line} boxes {next_block} cannot be matched to any clue')
        ...
        for index in range(len(known_boxes) - 1, 1, -1):
            ...
            if prev_clues and next_clues[-1].index == clues[-1].index:
                if not self._can_join(prev_block, block, clues[-1].value):
                    prev_clues.pop()

            if not prev_clues:
                raise ParadoxError(f'{self._line} boxes {prev_block} cannot be matched to any clue')
...
class NonogramSolver:
    ...
    def solve_line(self, clues: typing.Tuple[int], content: typing.List[CellType], line: Line=None) -> typing.Set[int]:
        line_solver = LineSolver(clues, content, line)
        line_solver.solve()
